...
public class BankController {
	...
	public Mono<ResponseEntity<Bank>> addBankUser(@Valid @RequestBody Bank newBankUser) {
		...
		return bankService.addBankUser(bankToSave).map(savedBankUser -> ResponseEntity.ok(savedBankUser));
	}
	...
	public Mono<ResponseEntity<BankDTO>> getBankUser(@PathVariable int id) {
		return bankService.getBankUserById(id).map(bankDTO -> ResponseEntity.ok(bankDTO))
	...
	public Mono<ResponseEntity<Bank>> updateBankUser(@PathVariable int id, @RequestBody Bank updatedBankUser) {
		return bankService.updateBankUser(id, updatedBankUser).map(updatedBank -> ResponseEntity.ok(updatedBank))
				.defaultIfEmpty(ResponseEntity.notFound().build());
	}
